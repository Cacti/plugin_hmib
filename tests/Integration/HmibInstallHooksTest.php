...
/*
 * Integration coverage for plugin_hmib_install(): verifies every hook and
 ...
 *
 * hmib_setup_table() include_once()s Cacti core's database.php via
...
it('registers every hook hmib depends on, its realms, and provisions its tables', function () {
	plugin_hmib_install();
	...
	foreach (array('config_arrays', 'config_settings', 'draw_navigation_text', 'poller_bottom', 'top_header_tabs', 'top_graph_header_tabs') as $expected) {
		expect($hooks)->toHaveKey($expected);
		expect($hooks[$expected]['name'])->toBe('hmib');
		expect($hooks[$expected]['file'])->toBe('setup.php');
	}

	$realms = array_column($GLOBALS['__test_registered_realms'], 'file');

	expect($realms)->toContain('hmib.php');
	expect($realms)->toContain('hmib_types.php');

	$sql = implode("\n", array_column($GLOBALS['__test_db_calls'], 'sql'));

	expect($sql)->toContain('plugin_hmib_hrDevices');
});
